...
function renderPage($conf, $pluginManager)
{
    ...
    foreach($common_hooks as $name) {
        ...
        $builder->assign('plugins_' . $name, $plugin_data);
    }

    $controllerClass = ucfirst($targetPage) . 'Controller';
    if (!class_exists($controllerClass)) {
        die('Internal error: controller class "' . $controllerClass . '" not found');
    }

    /** @var Controller $controller */
    $controller = new $controllerClass();
    $controller->setTpl($builder);
    $controller->setConf($conf);
    $controller->setPluginManager($pluginManager);
    $controller->setLinkDB($linkDB);
    ...
    $controller->setLoggedIn(isLoggedIn());
